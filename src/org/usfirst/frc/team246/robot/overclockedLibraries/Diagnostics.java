...
public class Diagnostics implements Runnable {
	...
	private static void sendVoltageAlert() {
		if (nearingVoltageMin && Timer.getFPGATimestamp() - voltageMessageTime >= 1) {
			String voltageAlert = String.format("Nearing the Voltage minimum. Voltage: %.2f", RobotMap.pdp.getVoltage());
	...
	private static void sendCurrentAlert(double secondsRemaining) {
		if (nearingCurrentMax && Timer.getFPGATimestamp() - currentMessageTime >= 1) {
			String currentAlert = String.format(
					...
					RobotMap.pdp.getTotalCurrent(), secondsRemaining);
	...
	private static void monitorVoltage() {
		nearingVoltageMin = RobotMap.pdp.getVoltage() - RobotMap.MAX_BROWNOUT_VOLTAGE < RobotMap.BROWNOUT_VOLTAGE_TOLERANCE;
		sendVoltageAlert();
		voltageLeeway = RobotMap.BROWNOUT_VOLTAGE_TOLERANCE - (RobotMap.pdp.getVoltage() - RobotMap.MAX_BROWNOUT_VOLTAGE);
	}
	...
	private static void monitorCurrent() {
		if (RobotMap.pdp.getTotalCurrent() > 120) {
