...
public class DataInterpolator {
	...
	/**
	 * This method can find a value based on piecewise linear functions
	 * generated from the data points entered.
	 * 
	 * @param value
	 *            The value whose y-coordinate you would like to return
	 * @return The value's y-coordinate on the corresponding piecewise linear
	 *         function
	...
	public static double interpolateValue(double value, double[][] dataArray)
		...
		if (dataArray == null) {
			...
			// defaults to the only y-coordinate entered if array has only 1 element
			return dataArray[0][1];
		}
		TwoDArrayQuickSorter.quickSort(dataArray, 0);
		// sort array by first value in each inner array
		if (value <= dataArray[0][0]) {
			// uses lowest two points if value is below range of array[i][0] values
			x1 = dataArray[0][0];
			x2 = dataArray[1][0];
			y1 = dataArray[0][1];
			y2 = dataArray[1][1];
		} else if (value >= dataArray[dataArray.length - 1][0]) { 
			// uses highest two points if value is above range of array[i][0] values
