 ...
 */
public class PIDController246 implements PIDInterface, LiveWindowSendable, Controller {
	...
	private double m_izone; // the izone
	private double m_maximumOutput = 1.0; // |maximum output|
	private double m_minimumOutput = -1.0; // |minimum output|
	private double m_maximumInput = 0.0; // maximum input - limit setpoint to this
	private double m_minimumInput = 0.0; // minimum input - limit setpoint to this
	private boolean m_continuous = false; // do the endpoints wrap around? e.g. Absolute encoder
	...
	private class PIDTask extends TimerTask {
		...
		private PIDController246 m_controller;

		public PIDTask(PIDController246 controller) {
			if (controller == null) {
				throw new NullPointerException("Given PIDController246 was null");
			}
	 ...
	 *            The default is 50ms.
	 */
	public PIDController246(double Kp, double Ki, double Kd, double Kf, double izone, PIDSource source, PIDOutput output,
			double period) {
	 ...
	 *            The default is 50ms.
	 */
	public PIDController246(double Kp, double Ki, double Kd, double Kf, PIDSource source, PIDOutput output, double period) {
	 ...
	 *            The default is 50ms.
	 */
	public PIDController246(double Kp, double Ki, double Kd, PIDSource source, PIDOutput output, double period) {
	 ...
	 *            The PIDOutput object that is set to the output percentage
	 */
	public PIDController246(double Kp, double Ki, double Kd, PIDSource source, PIDOutput output) {
	 ...
	 *            The PIDOutput object that is set to the output percentage
	 */
	public PIDController246(double Kp, double Ki, double Kd, double Kf, PIDSource source, PIDOutput output) {
		...
		if (enabled) {
			...
			synchronized (this) {
				...
				if (m_pidInput.getPIDSourceType().equals(PIDSourceType.kRate)) {
					...
					if (m_I != 0 && Math.abs(m_error) < m_izone && m_error*m_prevError > 0) {
						m_totalError += m_error;
					} else {
						m_totalError = 0;
					}
	 ...
	 * no synchronization because the PIDController246 class only calls it in
	 ...
	 *            the izone
	 */
	public synchronized void setPID(double p, double i, double d, double f, double izone) {
		m_P = p;
		m_I = i;
		m_D = d;
		m_F = f;
	...
	/**
	 * Set the setpoint for the PIDController246 Clears the queue for
	 * GetAvgError(). $
	 * 
	 * @param setpoint
	...
	/**
	 * Returns the current setpoint of the PIDController246 $
	 * 
	...
	/**
	 * Returns the change in setpoint over time of the PIDController246 $
	 * 
	...
	/**
	 * Returns whether or not any values have been collected. If no values have
	 * been collected, getAvgError is 0, which is invalid.
	 *
	 ...
	 * PIDController246.AbsoluteTolerance(0.1)) $
	 * 
	 * @param tolerance
	 ...
	 * 
	 * @param bufLength
	 *            Number of previous cycles to average.
	 */
	...
	/**
	 * Begin running the PIDController246
	 */
	@Override
	...
	/**
	 * Stop running the PIDController246, this sets the output to zero before
	 * stopping.
	 */
	@Override
	...
	/**
	 * Return true if PIDController246 is enabled.
	 *
